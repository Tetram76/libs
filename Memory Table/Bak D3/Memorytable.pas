...
uses SysUtils, Classes, Controls, Bde, DB, DBTables, DBCommon;
...
const
  ftNonTextTypes = [ftBytes, ftVarBytes, ftBlob, ftMemo, ftGraphic, ftFmtMemo, ftParadoxOle, ftDBaseOle, ftTypedBinary, ftCursor];
type
  Largeint = Longint;
...
uses DBConsts{, DBUtils}{, BdeUtils}, BDEConst, Forms{, MaxMin};

resourcestring
  SFieldUnsupportedType = 'Type du champ ''%s'' non supporté';
...
begin
  ...
  try
    if TableName = '' then
      AnsiToNative(Locale, '$InMem$', szTblName, SizeOf(szTblName) - 1)
    else
      AnsiToNative(Locale, TableName, szTblName, SizeOf(szTblName) - 1);
    FieldDescs := AllocMem(iFldCount * SizeOf(FLDDesc));
    for I := 0 to FieldDefs.Count - 1 do begin
      with FieldDefs[I] do
        EncodeFieldDesc(TFieldDescList(FieldDescs)[I], Name, DataType, Size);
    end;
...
procedure Register;
begin
  RegisterComponents('Persos', [TMemoryTable]);
end;
