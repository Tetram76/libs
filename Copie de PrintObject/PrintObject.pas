...
type
  ...
  private
    ...
    ResolutionEcran: Integer; { DPI de l'écran }
    ...
    function SetUp(Image: TBitmap): TBitmap;
    function GetResolutionPrinterX: Integer; { DPI de l'imprimante en X }
    function GetResolutionPrinterY: Integer; { DPI de l'imprimante en Y }
  ...
  published
    ...
    property ResolutionPrinterX: Integer read GetResolutionPrinterX;
    property ResolutionPrinterY: Integer read GetResolutionPrinterY;
    ...
    property ScreenResolution: Integer read ResolutionEcran;
...
begin
  ...
  ResolutionEcran := Screen.PixelsPerInch;
...
function TPrintObject.GetResolutionPrinterX: Integer; { DPI de l'imprimante en X }
begin
  Result := PrinterSettings.PixelsPerX;
end;
{-------------------------------------------------------------------------------}

function TPrintObject.GetResolutionPrinterY: Integer; { DPI de l'imprimante en Y }
begin
  Result := PrinterSettings.PixelsPerY;
end;
{-------------------------------------------------------------------------------}
...
function TPrintObject.SetUp;
begin
  Result := Image;
  with Result do begin
    case OptionSize of
      1: begin
          Height := PrinterSettings.PaperLengthPixels;
          Width := PrinterSettings.PaperWidthPixels;
          Canvas.Font.PixelsPerInch := ResolutionPrinterY;
        end;
      2: begin
          Height := MulDiv(PrinterSettings.PaperLengthPixels, ResolutionEcran, ResolutionPrinterY);
          Width := MulDiv(PrinterSettings.PaperWidthPixels, ResolutionEcran, ResolutionPrinterX);
        end;
    end;
  end;
end;
...
begin
  CheckPrinting;
  case FPreview of
    True: Result := TBitmap(OPreview.Pages[OPreview.Pages.Count - 1]).Canvas;
    else
...
begin
  ...
  if OptionSize = 2 then
    Result := MulDiv(Result, ResolutionEcran, ResolutionPrinterY);
end;
{-------------------------------------------------------------------------------}
...
begin
  ...
  if OptionSize = 2 then
    Result := MulDiv(Result, ResolutionEcran, ResolutionPrinterX);
end;
{-------------------------------------------------------------------------------}
...
begin
  ...
  if FPrintingState <> psStarting then begin
    ...
    WriteDateTime;
    case FPreview of
      True: OPreview.Pages.Add(SetUp(TBitmap.Create));
      else
...
function TPrintObject.OptionSize;
begin
  case FSizeOption of
    soPrinter: Result := 1;
    soScreen: Result := 2;
    else if (ResolutionEcran > ResolutionPrinterX) and (ResolutionEcran > ResolutionPrinterY) then
      Result := 1
    else
      Result := 2;
  end;
  if not FPreview then Result := 1;
end;
{-------------------------------------------------------------------------------}
...
begin
  ...
  if OptionSize = 2 then
    Result := Result * ResolutionPrinterY / ResolutionEcran;
end;
{-------------------------------------------------------------------------------}
...
begin
  ...
  if OptionSize = 2 then
    Result := Result * ResolutionPrinterX / ResolutionEcran;
end;
{-------------------------------------------------------------------------------}
...
begin
  ...
  case FPreview of
    True: begin
        Titre := Title;
        OPreview.Pages.Add(SetUp(TBitmap.Create));
        ...
        PrinterSettings.UsedPrinter.Canvas.Font.PixelsPerInch := ResolutionPrinterX;
      end;
  end;
