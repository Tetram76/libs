(*
 TRL 10/12/2007: ATTENTION VERSION MODIFIEE DE 1.564:

======================================================================
https://sourceforge.net/tracker/index.php?func=detail&aid=1841517&group_id=16012&atid=116012
----------------------------------------------------------------------
Access violation bug in last version 1.564!!!! (fix applied)

Access violation (Windows XP) at some pictures.
...
======================================================================
https://sourceforge.net/tracker/index.php?func=detail&aid=1559286&group_id=16012&atid=116012
----------------------------------------------------------------------
Folloging changes must be made for Version 1.564:

- Add OldImageHandle: HBitmap; as private-member for TChunkIHDR
- In the Constructor set   OldImageHandle := 0;
- Change procedure TChunkIHDR.FreeImageData to
...
begin
  ...
  if ImageDC     <> 0  then
  begin
    {$IFDEF UseDelphi}Self.Owner.Canvas.Handle := 0;{$ENDIF}
  ...
  {$ENDIF}
  ImageHandle := 0; OldImageHandle := 0;
  ImageDC := 0; ImageAlpha := nil; ImageData := nil;
  ImagePalette := 0; ExtraImageData := nil;
end;

- In TChunkIHDR.PrepareImageData add following line:
...
begin
  ...
  else if Size > 0 then {Only realloc if the new size is greater than 0}
...
begin
  ...
  begin
{$IFDEF UseDelphi}Self.Owner.Canvas.Handle := 0; {$ENDIF}
    SelectPalette(ImageDC, GetStockObject(DEFAULT_PALETTE),
      False);
    DeleteDC(ImageDC);
  end;
...
begin
  ...
  if Size > 256 then
    Owner.RaiseError(EPNGInvalidPalette,
...
begin
  ...
  if Header.HasPalette then
    PLTE := TChunkPLTE.Create(Self)
  else
    PLTE := nil;
  if Transparent then
    TRNS := TChunkTRNS.Create(Self)
  else
    TRNS := nil;
