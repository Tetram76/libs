...
begin
  Result := nil;
  for I := 0 to MethodPatches.Count - 1 do
    if TMethodPatch(MethodPatches[I]).Name = Name then
    begin
...
begin
  Result := False;
  Patch.Name := Name;
  if OldPtr <> NewPtr then
    with Patch do
    begin
      OldPointer := OldPtr;
      NewPointer := NewPtr;
      Move(PByte(OldPtr)^, OldBody[0], SizeOf(OldBody));
      if VirtualProtect(OldPtr, 16, PAGE_READWRITE, @Access) then
      begin
...
begin
  Result := False;
  if VirtualProtect(OldPointer, 16, PAGE_READWRITE, @Access) then
  begin
...
function TPngImageList.PngToIcon(const Png: TPNGImage; Background: TColor = clNone): HICON;
const
      ...
      begin
        ...
        begin
          ...
          if HasAlpha then
            ...
            ImageBits^[Y * Png.Width + X].rgbReserved := Integer(Color <> TransparencyColor) * 255;
        end;
      end;
...
begin
  ...
  try
    for I := 0 to FPngImages.Count - 1 do
    begin
      if TPngImageCollectionItem(FPngImages.Items[I]).PngImage = nil then
        Continue;
      if FEnabledImages or (FPngOptions = []) then
        Icon := PngToIcon(FPngImages.Items[I].PngImage, FPngImages.Items[I].Background)
      else
      begin
...
begin
  if FEnabledImages xor Value then
  begin
...
begin
  if Source is TPngImageCollectionItem then
  begin
