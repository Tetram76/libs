...
{idCGIRunner component supports now versions 8 and 9 of Indy}
{$I idRunner.inc}
  ...
  if WaitForSingleObject(ProcInfo.hProcess , FTimeOUT) = WAIT_TIMEOUT then
   ...
   if i > 0 then
    begin
      ...
      for tmp := 0 to ResponseInfo.RawHeaders.Count - 1 do
      begin
        if (ResponseInfo.RawHeaders.Names[tmp] = 'Set-Cookie') then
        begin
          Cookie := ResponseInfo.Cookies.Add;
          Cookie.CookieText := trim(copy(ResponseInfo.RawHeaders.Strings[tmp], pos(':', ResponseInfo.RawHeaders.Strings[tmp]) + 1, length(ResponseInfo.RawHeaders.Strings[tmp]) - pos(':', ResponseInfo.RawHeaders.Strings[tmp])));
        end;
      end;
      {$ENDIF}
