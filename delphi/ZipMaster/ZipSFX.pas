...
//      raise EDelphiZip.CreateResDisp(SF_StringToLong, True);
    ...
    begin
      ...
      begin
        AnsiStrings.StrPCopy(p, AnsiString(FDialogTitle));
        p := p + CaptionSize;
      end;
      if PathSize > 0 then
      begin
        AnsiStrings.StrPCopy(p, AnsiString(FDefaultExtractPath));
        p := p + PathSize;
      end;
      if CmdLineSize > 0 then
      begin
        AnsiStrings.StrPCopy(p, AnsiString(FCommandLine));
        p := p + CmdLineSize;
      end;
      if RegFailPathSize > 0 then
      begin
        AnsiStrings.StrPCopy(p, AnsiString(FFallbackExtractPath));
        p := p + RegFailPathSize;
      end;
      if StartMsgSize > 0
      then
        AnsiStrings.StrPCopy(p, AnsiString(FMessage));
