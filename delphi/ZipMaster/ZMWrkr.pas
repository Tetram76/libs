...
uses
  SFXInterface, ZMCompat, ZMUtils, ZMDlg, ZMCtx, ZMMsgStr, ZipMsg, AnsiStrings;
...
type
  ...
  MDZipData = record                        // MyDirZipData
    ...
    FileNameA: array[0..254] of AnsiChar;  // Array of current filename
    CRC32: Longword;
    ComprSize: Longword;
...
function MDZDGetName(const prec: pMZipData): string;
var
  StrA: AnsiString;
begin
  StrA := AnsiString(prec^.FileNameA);
  Result := String(StrA);
end;

procedure MDZDSetNameP(prec: pMZipData; const src: PAnsiChar; Len: Integer);
begin
  if Len > 253 then
    Len := 253;
  AnsiStrings.StrLCopy(prec^.FileNameA, src, Len);
  prec^.FileNameLen := Len;
end;

procedure MDZDSetName(prec: pMZipData; const Value: String{; MaxLen: Integer});
var
  StrA: AnsiString;
  len: integer;
begin
  StrA := AnsiString(Value);
  len := Length(StrA);
//  if len > MaxLen then
//    len := MaxLen;
  MDZDSetNameP(prec, PAnsiChar(StrA), len);
//  if len > 253 then
//    len := 253;
//  if len > prec^.FileNameLen then
//    len := prec^.FileNameLen;
//  StrPLCopy(prec^.FileNameA, StrA, len);
end;

//procedure MDZDSetNameS(prec: pMZipData; const Value: String); // overload;
//var
//  StrA: AnsiString;
//begin
//  StrA := AnsiString(Value);
//  MDZDSetName(prec, PAnsiChar(StrA), prec^.FileNameLen);
//end;
...
//    if CompareText(fname, MDZDp^.FileName) = 0 then
    if CompareText(fname, MDZDGetName(MDZDp)) = 0 then
...
var
  MDZDp: pMZipData;
  nameA:  array[0..254] of AnsiChar;
  fname: String;
begin
  if len > 253 then
    len := 253;
  AnsiStrings.StrLCopy(@NameA, hname, len);
  fname := String(PAnsiChar(@nameA[0]));
  for Result := 0 to (Count - 1) do
  begin
    MDZDp := Items[Result];
//    if CompareText(fname, MDZDp^.FileName) = 0 then
...
//          StrPLCopy(pFDS.fFileSpec, fFSpecArgs[i], Length(fFSpecArgs[i]) + 1);
    ...
    with ZipRecP^ do
    begin
      AnsiStrings.StrDispose(pZipFN);
      AnsiStrings.StrDispose(pZipPassword);
      AnsiStrings.StrDispose(pSuffix);
      AnsiStrings.StrDispose(fTempPath);
      AnsiStrings.StrDispose(fArchComment);
      for i := (fArgc - 1) downto 0 do
      begin
        pFDS := fFDS;
        Inc(pFDS, i);
        AnsiStrings.StrDispose(pFDS.fFileSpec);
      end;
      FreeMem(fFDS);
      ...
      begin
        pExFiles := fExFiles;
        Inc(pExFiles, i);
        AnsiStrings.StrDispose(pExFiles.fFileSpec);
      end;
      FreeMem(fExFiles); 
      FreeMem(ZipRecP);
    end;
    ...
    begin
      case ActionCode of
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if IsChanged then
              begin
                AnsiStrings.StrPLCopy(ZCallBackRec^.Msg, AnsiString(OldFileName), 512);
        ...
        begin
          ...
          if pwd <> '' then
          begin
            AnsiStrings.StrPLCopy(ZCallBackRec^.Msg, AnsiString(pwd), PWLEN);
            ZCallBackRec^.ActionCode := -1;
          end
          else
          begin      // no password
        ...
        zacComment:
          ...
          begin
            ...
            begin
              ...
              if IsChanged then
                if (FileComment <> '') then
                  AnsiStrings.StrPLCopy(ZCallBackRec^.Msg, AnsiString(FileComment), 511)
                else
...
//            StrLCopy(pFDta.fPassword, pZipPassword, Length(pZipPassword));
          ...
          begin                             // Found...
            ...
            AnsiStrings.StrDispose(pFDta.fPassword);
            pFDta.fPassword := Nil;
...
//            StrPLCopy(pFDta.fFileSpec, tmp, Length(tmp) + 1);
    ...
    begin
      ...
      AnsiStrings.StrDispose(fTempPath);
      AnsiStrings.StrDispose(fArchComment);
      AnsiStrings.StrDispose(fGRootDir);
      for i := (fArgc - 1) downto 0 do
      begin
        pFDta := fFDS;
        Inc(pFDta, i);
        AnsiStrings.StrDispose(pFDta.fFileSpec);
        AnsiStrings.StrDispose(pFDta.fPassword);
        AnsiStrings.StrDispose(pFDta.fRootDir);
      end;
      FreeMem(fFDS);
      for i := (fTotExFileSpecs - 1) downto 0 do
      begin
...
//                if AnsiStrIComp(PAnsiChar(FFSpecArgs.Strings[0]),
//                  PAnsiChar(FileName)) = 0 then
                begin                         // Found?
        ...
        begin
          ...
          if (fExtractDir <> Nil) then
            AnsiStrings.StrDispose(fExtractDir);

          for i := (fArgc - 1) downto 0 do
          begin
            pUFDS := fUFDS;
            Inc(pUFDS, i);
            AnsiStrings.StrDispose(pUFDS.fFileSpec);
          end;
          FreeMem(fUFDS);
        end;
        FreeMem(UnzRecP);
...
{$ENDIF}
    ...
    begin
      ...
      if FileWrite(FInFileHandle, PAnsiChar(CommentBuf)^, len) <> len then
...
{    case FNumbering of
    ...
    if FileExists(DiskFile) then
      if FileAge(DiskFile, fileTimeStamp) and (fileTimeStamp = FileDateToDateTime(FDateStamp)) and (Pred(DiskSeq) < FDiskNr) then
      begin
